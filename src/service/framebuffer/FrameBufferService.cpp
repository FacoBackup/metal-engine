...
namespace Metal {
   void FrameBufferService::createSampler(bool linear, VkSampler &vkImageSampler) {
        ...
        VulkanUtils::CheckVKResult(vkCreateSampler(ApplicationContext::Get().vulkanContext.device.device, &samplerCreateInfo, nullptr,
                                                   &vkImageSampler));
    }
    ...
    void FrameBufferService::createDepthAttachment(FrameBufferInstance *framebuffer) const {
        VkFormat depthFormat = VulkanUtils::GetValidDepthFormat(ApplicationContext::Get().vulkanContext.physDevice.physical_device);
    ...
    std::shared_ptr<FrameBufferAttachment> FrameBufferService::createAttachmentInternal(
        ...
        VulkanUtils::CheckVKResult(vkCreateImage(ApplicationContext::Get().vulkanContext.device.device, &image, nullptr, &attachment->vkImage));

        VkMemoryAllocateInfo memAlloc{};
        VkMemoryRequirements memReqs;
        vkGetImageMemoryRequirements(ApplicationContext::Get().vulkanContext.device.device, attachment->vkImage, &memReqs);
        memAlloc.allocationSize = memReqs.size;
        memAlloc.memoryTypeIndex = VulkanUtils::GetMemTypeIndex(ApplicationContext::Get().vulkanContext.physicalDeviceMemoryProperties,
                                                                memReqs.memoryTypeBits,
                                                                VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

        VulkanUtils::CheckVKResult(vkAllocateMemory(ApplicationContext::Get().vulkanContext.device.device, &memAlloc, nullptr,
                                                    &attachment->vkImageMemory));
        VulkanUtils::CheckVKResult(
            vkBindImageMemory(ApplicationContext::Get().vulkanContext.device.device, attachment->vkImage, attachment->vkImageMemory, 0));
        ...
        VulkanUtils::CheckVKResult(vkCreateImageView(ApplicationContext::Get().vulkanContext.device.device, &imageView, nullptr,
    ...
    void FrameBufferService::createRenderPass(FrameBufferInstance *framebuffer) const {
        ...
        VulkanUtils::CheckVKResult(vkCreateRenderPass(ApplicationContext::Get().vulkanContext.device.device,
                                                      &renderPassInfo,
                                                      nullptr,
    ...
    void FrameBufferService::createVKFrameBuffer(FrameBufferInstance *framebuffer) const {
        ...
        vkCreateFramebuffer(ApplicationContext::Get().vulkanContext.device.device, &framebufferInfo, nullptr,
