...
namespace Metal {
    ...
    void DockService::buildViews(ImGuiID &windowId, Metal::AbstractPanel *panel) {
        ...
        dockRepository.center.direction = DockPosition::CENTER;
        ...
        for (size_t i = 0; i < left.size(); i++) {
            ...
            dockSpace.direction = DockPosition::LEFT;
        ...
        for (size_t i = 0; i < right.size(); i++) {
            ...
            dockSpace.splitDir = ImGuiDir_Down;
            dockSpace.direction = DockPosition::RIGHT;
            createDockSpace(dockSpace, windowId);
            addWindow(dockSpace, panel);
        }
        ...
        for (size_t i = 0, bottomSize = bottom.size(); i < bottomSize; i++) {
            ...
            dockSpace.direction = DockPosition::BOTTOM;
            createDockSpace(dockSpace, windowId);
            addWindow(dockSpace, panel);
        }
    ...
    void DockService::prepareForRemoval(DockDTO &dock, DockSpacePanel *dockSpacePanel) {
        DockRepository &dockRepository = context.getEditorContext().dockRepository;
    ...
    void DockService::updateForRemoval(AbstractPanel *panel) {
        DockRepository &dockRepository = context.getEditorContext().dockRepository;
        ...
        if (dockRepository.dockPanelToRemove != nullptr) {
            ...
            vec.erase(std::remove(vec.begin(), vec.end(), dockRepository.dockPanelToRemove), vec.end());
