...
namespace Metal {
    ...
    bool DebugPhongPass::shouldRun() {
        return vulkanContext->isRayTracingSupported();
    }

    void DebugPhongPass::onSync() {
        worldRepository->registry.view<PrimitiveComponent, TransformComponent>().each([&](entt::entity entityId, const PrimitiveComponent& mesh, const TransformComponent& transform) {
