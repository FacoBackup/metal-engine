...
namespace Metal {
    void CardDirectoryPanel::renderCard(const std::shared_ptr<FSEntry> &entry) {
        ImGui::BeginGroup();
        
        bool isSelected = filesContext.selected.contains(entry->getId());
        bool interactable = isInteractable(entry);
        ...
        const ImGuiChildFlags childFlags = ImGuiChildFlags_Border | ImGuiChildFlags_FrameStyle | ImGuiChildFlags_FrameStyle;
        const ImGuiWindowFlags windowFlags = ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse;
        
        if (ImGui::BeginChild(entry->getId().c_str(), CARD_SIZE, childFlags, windowFlags)) {
            if (isSelected) {
            ...
            static const std::vector<std::string> textureExtensions = TEXTURE_EXTENSIONS;
            if (!entry->isDirectory && std::ranges::find(textureExtensions, entry->extension) != textureExtensions.end()) {
                auto *texture = textureService->stream(entry->getId());
                if (texture != nullptr) {
        ...
        if (ImGui::IsItemClicked()) {
            if (ImGui::IsKeyDown(ImGuiMod_Ctrl)) {
                if (isSelected) filesContext.selected.erase(entry->getId());
                else filesContext.selected.insert({entry->getId(), entry});
            } else {
        ...
        if (ImGui::BeginChild((id + "card_view").c_str(), ImVec2(0, 0))) {
            ...
            for (const auto &child : filesContext.currentDirectory->children) {
                if (matchesSearch(child)) {
                    ...
                    float next_button_x2 = last_button_x2 + style.ItemSpacing.x + CARD_WIDTH; // Expected position if next button was on same line
