...
#include "../../../../renderer/data/SceneData.h"
...
namespace Metal {
    ...
    std::function<void()> FilesPanel::onAction() {
        return [this]() {
            ...
            for (const std::string &file: files) {
                if (SINGLETONS.meshImporter.isCompatible(file)) {
                    SINGLETONS.meshImporter.importScene(filesContext.currentDirectory->absolutePath,
                                                      file);
                } else if (SINGLETONS.textureImporter.isCompatible(file)) {
                    SINGLETONS.textureImporter.importTexture(filesContext.currentDirectory->absolutePath, file);
                }
            }
    ...
    void FilesPanel::onInitialize() {
        filesContext.setCurrentDirectory(SINGLETONS.filesService.getRoot());
            ...
            for (auto &child: filesContext.currentDirectory->children) {
                if (filesContext.filterType == EntryType::NONE || child->type == filesContext.filterType) {
                    ...
                    ImGui::PushStyleColor(ImGuiCol_ChildBg, isSelected
                                                                ? SINGLETONS.editorRepository.accent
                                                                : SINGLETONS.themeService.palette0);
                    renderItem(child);
                    ImGui::PopStyleColor();
    ...
    void FilesPanel::renderItem(FileEntry *root) {
        ...
        if (ImGui::BeginChild(root->getId().c_str(), UIUtil::AUX_VEC2, ImGuiChildFlags_Border)) {
            ...
            onClick(root);
            handleDragDrop(root);
            ImGui::PushFont(SINGLETONS.guiContext.getLargeIconsFont());
            ...
            if (root->type == EntryType::DIRECTORY) {
                ...
                auto *texture = SINGLETONS.streamingRepository.streamTexture(
                    root->getId(), LevelOfDetail::LOD_3);
                if (texture != nullptr) {
                    SINGLETONS.guiContext.renderImage(texture, CARD_SIZE - 8, CARD_SIZE - 22);
                }
            } else {
    ...
    void FilesPanel::deleteSelected() const {
        SINGLETONS.filesService.deleteFiles(filesContext.selected);
        FilesService::GetEntries(filesContext.currentDirectory);
    }
    ...
    void FilesPanel::openResource(FileEntry *root) {
        switch (root->type) {
            case EntryType::MESH: {
                SINGLETONS.meshService.createMeshEntity(root->name, root->getId());
                break;
            }
            case EntryType::SCENE: {
                SINGLETONS.meshService.createSceneEntities(root->getId());
                break;
            }
