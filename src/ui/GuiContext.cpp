...
#include "../renderer/data/TextureInstance.h"
...
namespace Metal
{
    ...
    {
        SINGLETONS.descriptorService.setImageDescriptor(texture);
    ...
    void GuiContext::onInitialize()
    {
        ...
        ImGui_ImplGlfw_InitForVulkan(SINGLETONS.glfwContext.getWindow(), true);
        ImGui_ImplVulkan_InitInfo init_info = {};
        init_info.Instance = SINGLETONS.vulkanContext.instance.instance;
        init_info.PhysicalDevice = SINGLETONS.vulkanContext.physDevice.physical_device;
        init_info.Device = SINGLETONS.vulkanContext.device.device;
        init_info.QueueFamily = SINGLETONS.vulkanContext.queueFamily;
        init_info.Queue = SINGLETONS.vulkanContext.graphicsQueue;
        init_info.PipelineCache = SINGLETONS.vulkanContext.pipelineCache;
        init_info.DescriptorPool = SINGLETONS.vulkanContext.descriptorPool;
        init_info.RenderPass = SINGLETONS.vulkanContext.imguiVulkanWindow.RenderPass;
        init_info.Subpass = 0;
        init_info.MinImageCount = MAX_FRAMES_IN_FLIGHT;
        init_info.ImageCount = SINGLETONS.vulkanContext.imguiVulkanWindow.ImageCount;
    ...
    {
        const VkResult err = vkDeviceWaitIdle(SINGLETONS.vulkanContext.device.device);
        VulkanUtils::CheckVKResult(err);
        ImGui_ImplVulkan_Shutdown();
        ImGui_ImplGlfw_Shutdown();
        ImGui::DestroyContext();
        ImGui_ImplVulkanH_DestroyWindow(SINGLETONS.vulkanContext.instance, SINGLETONS.vulkanContext.device.device,
                                        &SINGLETONS.glfwContext.getGUIWindow(),
                                        nullptr);
    }
}
