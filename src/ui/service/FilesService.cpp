...
std::filesystem::remove_all(ContextProvider::Get().getAssetDirectory() + F(entry.second->getId(), LevelOfDetail::LOD_0));\
std::filesystem::remove_all(ContextProvider::Get().getAssetDirectory() + F(entry.second->getId(), LevelOfDetail::LOD_1));\
std::filesystem::remove_all(ContextProvider::Get().getAssetDirectory() + F(entry.second->getId(), LevelOfDetail::LOD_2));\
std::filesystem::remove_all(ContextProvider::Get().getAssetDirectory() + F(entry.second->getId(), LevelOfDetail::LOD_3));

#define DELETE_S(F)\
std::filesystem::remove_all(entry.second->absolutePath);\
std::filesystem::remove_all(ContextProvider::Get().getAssetDirectory() + F(entry.second->getId()));
...
namespace Metal {
    void FilesService::onInitialize() {
        root = new FileEntry(nullptr, ContextProvider::Get().getAssetRefDirectory(), "", "");
    ...
    void FilesService::createMaterial(std::string targetDir) const {
        ...
        MaterialInfo data{};
        DUMP_TEMPLATE(ContextProvider::Get().getAssetDirectory() + FORMAT_FILE_MATERIAL(materialMetadata.getId()), data)
    }
