...
namespace Metal {
    void FrameBufferService::createSampler(bool linear, VkSampler &vkImageSampler) {
        ...
        VulkanUtils::CheckVKResult(vkCreateSampler(SINGLETONS.vulkanContext.device.device, &samplerCreateInfo, nullptr,
                                                   &vkImageSampler));
    }
    ...
    void FrameBufferService::createDepthAttachment(FrameBufferInstance *framebuffer) const {
        VkFormat depthFormat = VulkanUtils::GetValidDepthFormat(SINGLETONS.vulkanContext.physDevice.physical_device);
    ...
    std::shared_ptr<FrameBufferAttachment> FrameBufferService::createAttachmentInternal(
        ...
        VulkanUtils::CheckVKResult(vkCreateImage(SINGLETONS.vulkanContext.device.device, &image, nullptr, &attachment->vkImage));

        VkMemoryAllocateInfo memAlloc{};
        VkMemoryRequirements memReqs;
        vkGetImageMemoryRequirements(SINGLETONS.vulkanContext.device.device, attachment->vkImage, &memReqs);
        memAlloc.allocationSize = memReqs.size;
        memAlloc.memoryTypeIndex = VulkanUtils::GetMemTypeIndex(SINGLETONS.vulkanContext.physicalDeviceMemoryProperties,
                                                                memReqs.memoryTypeBits,
                                                                VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

        VulkanUtils::CheckVKResult(vkAllocateMemory(SINGLETONS.vulkanContext.device.device, &memAlloc, nullptr,
                                                    &attachment->vkImageMemory));
        VulkanUtils::CheckVKResult(
            vkBindImageMemory(SINGLETONS.vulkanContext.device.device, attachment->vkImage, attachment->vkImageMemory, 0));
        ...
        VulkanUtils::CheckVKResult(vkCreateImageView(SINGLETONS.vulkanContext.device.device, &imageView, nullptr,
    ...
    void FrameBufferService::createRenderPass(FrameBufferInstance *framebuffer) const {
        ...
        VulkanUtils::CheckVKResult(vkCreateRenderPass(SINGLETONS.vulkanContext.device.device,
                                                      &renderPassInfo,
                                                      nullptr,
    ...
    void FrameBufferService::createVKFrameBuffer(FrameBufferInstance *framebuffer) const {
        ...
        vkCreateFramebuffer(SINGLETONS.vulkanContext.device.device, &framebufferInfo, nullptr,
