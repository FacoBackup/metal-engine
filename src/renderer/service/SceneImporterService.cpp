...
#define TEXTURE_P SINGLETONS.textureImporter.importTexture(targetDir, rootDirectory + "/" + texturePath.data)
...
namespace Metal
{
    std::string SceneImporterService::persistMesh(const std::string& targetDir, const MeshData& mesh) const
    {
        ...
        DUMP_TEMPLATE(ContextProvider::Get().getAssetDirectory() + FORMAT_FILE_MESH(metadata.getId(), LevelOfDetail::LOD_0), mesh)
    ...
    void SceneImporterService::importScene(const std::string& targetDir, const std::string& pathToFile) const
    {
        ...
        DUMP_TEMPLATE(ContextProvider::Get().getAssetDirectory() + FORMAT_FILE_SCENE(sceneMetadata.getId()), sceneData)
    }
                                            ...
                                            const LevelOfDetail& levelOfDetail) const
    {
        ...
        DUMP_TEMPLATE(ContextProvider::Get().getAssetDirectory() + FORMAT_FILE_MESH(fileId, levelOfDetail), simplifiedMesh)
    }
