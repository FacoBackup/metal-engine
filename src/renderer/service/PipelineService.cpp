...
namespace Metal {
    void PipelineService::createPipelineLayout(const std::vector<DescriptorInstance *> &descriptorSetsToBind,
        ...
        VulkanUtils::CheckVKResult(vkCreatePipelineLayout(SINGLETONS.vulkanContext.device.device, &layoutInfo, nullptr,
    ...
    PipelineInstance *PipelineService::createComputePipeline(const PipelineBuilder &pipelineBuilder) const {
        ...
        if (vkCreateComputePipelines(SINGLETONS.vulkanContext.device.device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr,
        ...
        vkDestroyShaderModule(SINGLETONS.vulkanContext.device.device, computeShaderModule, nullptr);
        return pipeline;
    }
    ...
    PipelineInstance *PipelineService::createRenderingPipeline(PipelineBuilder &pipelineBuilder) {
        ...
        VulkanUtils::CheckVKResult(vkCreateGraphicsPipelines(SINGLETONS.vulkanContext.device.device, VK_NULL_HANDLE, 1,
                                                             &pipelineInfo,
                                                             nullptr,
                                                             &pipeline->vkPipeline));
        ...
        vkDestroyShaderModule(SINGLETONS.vulkanContext.device.device, fragmentShaderModule, nullptr);
        vkDestroyShaderModule(SINGLETONS.vulkanContext.device.device, vertexShaderModule, nullptr);
        return pipeline;
    }
} // Metal
