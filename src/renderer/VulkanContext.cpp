...
namespace Metal {
    ...
    void VulkanContext::createPresentMode() {
         VkPresentModeKHR presentModes = !SINGLETONS.engineRepository.vsync ? VK_PRESENT_MODE_IMMEDIATE_KHR : VK_PRESENT_MODE_FIFO_KHR;
    ...
    void VulkanContext::addExtensions(vkb::InstanceBuilder &instanceBuilder,
        ...
        if (auto sysInfoResult = vkb::SystemInfo::get_system_info(); sysInfoResult) {
            ...
            if (sysInfo.validation_layers_available && ContextProvider::Get().isDebugMode()) {
    ...
    void VulkanContext::onInitialize() {
        ...
        this->window = SINGLETONS.glfwContext.getWindow();
        vkb::InstanceBuilder instanceBuilder;

        // ------- CORE INITIALIZATION
        // ----- INSTANCE AND EXTENSIONS
        addExtensions(instanceBuilder, SINGLETONS.glfwContext.getInstanceExtensions());
        ...
        SINGLETONS.coreBuffers.onInitialize();
        SINGLETONS.coreFrameBuffers.onInitialize();
        SINGLETONS.coreTextures.onInitialize();
        SINGLETONS.coreDescriptorSets.onInitialize();
        // ------- REPOSITORY INITIALIZATION
    }

    void VulkanContext::dispose() const {
        SINGLETONS.pipelineService.disposeAll();
        SINGLETONS.textureService.disposeAll();
        SINGLETONS.meshService.disposeAll();
        SINGLETONS.framebufferService.disposeAll();
        vkDestroyDescriptorPool(SINGLETONS.vulkanContext.device.device, descriptorPool,
                                nullptr);
        vkDestroyCommandPool(SINGLETONS.vulkanContext.device.device, commandPool,
                             nullptr);
    ...
    void VulkanContext::createDescriptorPool() const {
        ...
        VulkanUtils::CheckVKResult(vkCreateDescriptorPool(device.device, &poolInfo,
                                                          nullptr, &SINGLETONS.vulkanContext.descriptorPool));
    }
    ...
    void VulkanContext::submitFrame(VkSemaphore image_acquired_semaphore, VkSemaphore render_complete_semaphore,
        ...
        VkSubmitInfo info = {};
        SINGLETONS.vulkanContext.pushCommandBuffer(fd->CommandBuffer);
        info.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;
        info.waitSemaphoreCount = 1;
        info.pWaitSemaphores = &image_acquired_semaphore;
        info.pWaitDstStageMask = &wait_stage;
        info.commandBufferCount = SINGLETONS.vulkanContext.getCommandBuffers().size();
        info.pCommandBuffers = SINGLETONS.vulkanContext.getCommandBuffers().data();
        info.signalSemaphoreCount = 1;
        info.pSignalSemaphores = &render_complete_semaphore;
        VulkanUtils::CheckVKResult(vkEndCommandBuffer(fd->CommandBuffer));
        VulkanUtils::CheckVKResult(vkQueueSubmit(SINGLETONS.vulkanContext.graphicsQueue, 1, &info, fd->Fence));
    }
}