...
namespace Metal
{
    void CommandBufferRecorder::createCommandBuffer()
    {
        ...
        allocInfo.commandPool = SINGLETONS.vulkanContext.commandPool;
        allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
        allocInfo.commandBufferCount = MAX_FRAMES_IN_FLIGHT;
        VulkanUtils::CheckVKResult(
            vkAllocateCommandBuffers(SINGLETONS.vulkanContext.device.device, &allocInfo, _commandBuffers.data()));
    }
    ...
    {
        auto vkCommandBuffer = _commandBuffers[ContextProvider::Get().getFrameIndex()];
        ...
        SINGLETONS.vulkanContext.pushCommandBuffer(vkCommandBuffer);
    }
