...
namespace Metal {
    void DescriptorInstance::dispose() const {
        ...
        vkDestroyDescriptorSetLayout(SINGLETONS.vulkanContext.device.device, vkDescriptorSetLayout, nullptr);
    }
    ...
    void DescriptorInstance::Write(const VkDescriptorSet &vkDescriptorSet,
        ...
        vkUpdateDescriptorSets(SINGLETONS.vulkanContext.device.device,
    ...
    void DescriptorInstance::create() {
        ...
        VulkanUtils::CheckVKResult(vkCreateDescriptorSetLayout(SINGLETONS.vulkanContext.device.device, &layoutInfo,
                                                               nullptr,
                                                               &vkDescriptorSetLayout));

        VkDescriptorSetAllocateInfo allocInfo = {};
        allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
        allocInfo.descriptorPool = SINGLETONS.vulkanContext.descriptorPool; // Created during setup
        allocInfo.descriptorSetCount = 1;
        allocInfo.pSetLayouts = &vkDescriptorSetLayout;

        VulkanUtils::CheckVKResult(vkAllocateDescriptorSets(SINGLETONS.vulkanContext.device.device, &allocInfo, &vkDescriptorSet));
