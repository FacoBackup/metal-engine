...
namespace Metal {
    MeshBVH::MeshBVH(MeshData *mesh) {
        ...
        for (size_t i = 0; i < allTriangles.size(); i++) {
            TriangleBVH buildTri = allTriangles[i];
    ...
    void MeshBVH::split(int parentIndex, int triGlobalStart, int triNum, int depth) {
        BLASBuffer parent = allNodes[parentIndex];
        ...
        if (split.cost < parentCost && depth < MAX_DEPTH) {
            BoundingBoxBVH boundsLeft, boundsRight;
            int numOnLeft = 0;

            // Partition triangles
            for (int i = triGlobalStart; i < triGlobalStart + triNum; ++i) {
                TriangleBVH &tri = allTriangles[i];
    ...
    SplitResult MeshBVH::ChooseSplit(const BLASBuffer &node, int start, int count) {
    ...
    float MeshBVH::EvaluateSplit(int splitAxis, float splitPos, int start, int count) {
        BoundingBoxBVH left, right;
        int numLeft = 0, numRight = 0;

        for (int i = start; i < start + count; ++i) {
            const TriangleBVH &tri = allTriangles[i];
