...
namespace Metal {
    void CameraService::onSync() {
        camera = &SINGLETONS.worldRepository.camera;
        if (camera != nullptr) {
            updateAspectRatio();
            if (camera->isNotFrozen()) {
                updateMatrices();
                SINGLETONS.engineContext.setCameraUpdated(true);
    ...
    void CameraService::updateAspectRatio() const {
        const auto &runtimeRepository = SINGLETONS.runtimeRepository;
    ...
    void CameraService::handleInputInternal() const {
        ...
        const float multiplier = 10 * camera->movementSensitivity *
                                 SINGLETONS.engineContext.deltaTime;
